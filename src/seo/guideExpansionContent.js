const englishGuides = {
  ...
  makeCircle: {
    title: 'How to make a circle crosshair in VALORANT', intro: 'Use compact line layers to frame the center, then compare the result with working circle codes before you save it.', summary: 'A circle is an approximation built from short, closed line layers. Keep the horizontal and vertical values balanced, close the offset, and preview at normal scale because a ring that looks perfect when enlarged may look square in game.',
    sections: [
    ...
    metaTitle: 'How to Make a Circle Crosshair in VALORANT | AimCodes', metaDescription: 'Learn the settings behind a VALORANT circle crosshair, compare hollow and center-dot versions, and preview working circle codes.',
  },
  movementError: {
...
const localizedGuides = {
  es: {
    ...
    makeCircle: ['Cómo hacer una mira circular en VALORANT', 'Equilibra líneas cortas y un desplazamiento cerrado para formar un aro pequeño.', 'La forma circular es una aproximación con líneas. Compárala a escala normal con códigos que ya funcionan.', ['Construir un aro simétrico', 'Decidir si usar punto central', 'Comparar con un código verificado']],
  ...
  'pt-BR': {
    ...
    makeCircle: ['Como fazer uma mira circular no VALORANT', 'Equilibre linhas curtas e deslocamento fechado para formar um anel pequeno.', 'O círculo é uma aproximação com linhas. Compare em escala normal com códigos que funcionam.', ['Construir um anel simétrico', 'Decidir sobre o ponto central', 'Comparar com código verificado']],
  ...
  'zh-CN': {
    ...
    makeCircle: ['无畏契约圆形准星怎么调', '用对称短线和闭合偏移拼出小圆环，再按正常比例检查。', '圆形是像素线条形成的近似效果，请和可用代码放在同一地图里对比。', ['拼出对称圆环', '决定是否保留中心点', '和已验证代码比较']],
