...
## 2026-09-11 SEO 范围收缩与质量门（本地待发布）

产品界面继续支持英语、西班牙语、巴西葡萄牙语、简体中文和日语，但 SEO 不再平均分配：英语、日语负责增长；西语、葡语只维护已有赢家；中文保留完整可用界面，但仅首页、准星库和反应测试参与索引。中文内容型页面继续可访问并使用 `noindex,follow`。

近义词扩页改为搜索意图门。`meme-crosshairs` 与 Funny 内容完全重叠，因此永久重定向至 Funny；职业选手资料库和职业聚合页永久重定向至准星库。已有具体选手准星代码页因为具备搜索表现和“预览、测试、复制”的真实任务而保留，但删除人物资料层。完整规则见 `docs/SEO_OPERATING_POLICY.md`。
...
GSC 最近完整 14 天达到 527 次点击、22,430 次展示、CTR 2.35%，较前 14 天分别增长 454.7%、201.5% 与 84.0%（相对）。查询中已经出现 `tracker crosshair`、`double crosshair valorant` 与 `plus crosshair valorant`，但站内没有对应集合页；Dot 词簇则已有 1,413 次展示，需要更多真实可选造型而不是再拆一个近义集合。

本轮新增 Tracker、Double Cross、Plus 三个独立集合，并在 Tap Dot、Tracker、Twin Line 三个原创形态家族中新增 60 个几何不同的可索引准星。五语种共增加 315 个 canonical URL。刚扩到 100 款的 Funny 集合和现有 Small、Dot 赢家 URL 保持不变，避免短期反复改写或关键词内耗。数据、实施范围与复查门槛见 `docs/SEO_DATA_REVIEW_2026-09-03.md`。
...
截至 GSC 2026-08-18 完整数据，最近 14 天获得 108 次点击、8,408 次展示，CTR 1.28%、平均排名 26.47。GA4 同期自然搜索带来 178 名用户、互动率 65.9%，已成为规模最大且质量最高的可识别渠道。

本轮保护 Small、One-Tap、Green、Phantom 与葡语 Dot 等赢家页，不重复改写刚更新的英语首页。重点强化排名 5–12 位但点击不足的 TenZ、复制队友准星、射击误差与 Tracker 查询，并在首页增加直接可见的热门入口，把用户和内链权重送往这些现有页面。完整数据、实施范围与观察门槛见 `docs/SEO_DATA_REVIEW_2026-08-21.md`。

## 2026-08-17 GSC 点击率优化

截至 GSC 2026-08-14 完整数据，站点累计获得 60 次点击和 4,658 次展示，CTR 1.3%、平均排名 25.7。本轮不新增 URL，而是优先处理已经获得第一页曝光但尚未产生点击的搜索落地页。

英语首页重点承接 `valorant aim code` 变体；移动误差页直接回答 on / off；圆形集合继续负责现成代码，制作教程继续负责参数设置；导入报错页补齐 GSC 已出现的完整错误提示；Sato 页补齐 Settings 意图但不声称 latest/current。Small、Pink、Green、One-Tap 与葡语 Dot 当前有正向信号，本轮全部保持不变。完整数据、动作和观察门槛见 `docs/SEO_DATA_REVIEW_2026-08-17.md`。
...
- 4 个真实工具页 × 5 种语言；
- 396 个高价值准星详情 × 5 种语言；
- 合计 2,290 个主 Sitemap URL，其中 1,980 个准星详情同时进入独立准星 Sitemap；
...
- 发布本轮 1,560 URL 主 Sitemap 和 1,280 URL 独立准星 Sitemap；
