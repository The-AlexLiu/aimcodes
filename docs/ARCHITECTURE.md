...
2. `reactionRecommendation.js` 计算计分成绩、稳定性、最好/最慢成绩和提前点击。三轮中只有当最慢一轮与中位数相差至少 250ms，且该差距至少为另外两轮差距的 3 倍时，才会排除这一次孤立停顿；原始成绩继续保留在折叠明细中并标记为未计入。
3. 免费结果展示反应段位、瞄准风格、成绩明细、本地趋势和挑战分享，不直接展示具体推荐准星或代码。
4. `PremiumAimPackOffer` 在真实进入视口后记录曝光；付款前只用遮罩代码和通用地图示意解释交付形式。用户选择模式、视觉偏好和主要武器，并填写交付邮箱后才进入 Whop 结账；邮箱通过 Whop Checkout Embed 的预填能力复用，避免重复输入。
5. 只有服务端验证的付款事件可以创建五槽准星包权益。具体准星名称、逐款地图预览、完整可复制代码与两次重新生成只出现在私有交付页。
...
- `?qa=1` 阻止内部验收流量进入 GA4。
- `scripts/validate-analytics.mjs` 当前要求 51 个漏斗事件，覆盖反应测试、其行为触发引导、分享、付费偏好、结账退出和交付确认，并阻止业务组件绕过统一分享成功 helper 直接发送 `share`。反应测试入口统一记录为安全枚举 `finder_entry`，不采集搜索词或完整来源 URL。
