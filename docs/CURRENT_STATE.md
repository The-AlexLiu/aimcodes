...
- 1,468 个生成 HTML 路由；
- 848 个 sitemap canonical URL；
- 156 个可索引准星详情，另有 152 个可访问但保持 `noindex,follow` 的目录详情；
- 156 张索引准星独立预览图、156 张对应 OG 图和 25 张集合页 OG 图；
- 独立准星 Sitemap 覆盖 624 个四语种准星详情 URL；
- 图片 Sitemap 覆盖 724 个四语种集合/详情页，共声明 1,348 个图片引用；
- 社媒视频、方形图和配套封面生成工具。
...
- `pnpm validate:manifest` 统一检查完整目录、156 个索引记录和 25 个集合之间的关系；
