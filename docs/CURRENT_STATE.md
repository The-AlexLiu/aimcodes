...
- 5 种语言；
- 440 条准星源数据，去重后 432 种可见样式；职业选手同款允许共享外观但保留独立档案；
- 3 张地图预览；
- 3 轮反应测试；
- 9 个反应段位；
...
- 2,505 个生成 HTML 路由；
- 1,730 个 sitemap canonical URL；
- 288 个可索引准星详情，另有 152 个可访问但保持 `noindex,follow` 的目录详情；
- 288 张索引准星独立预览图、288 张对应 OG 图和 25 张集合页 OG 图；
- 独立准星 Sitemap 覆盖 1,440 个五语种准星详情 URL；
- 图片 Sitemap 覆盖 1,565 个五语种集合/详情页，共声明 3,005 个图片引用；
- 社媒视频、方形图和配套封面生成工具。
...
3. 第三批 100 个新详情发布后应观察 21–30 天的抓取、收录、展示和长尾词分布；未验证信号前不继续开放剩余 152 个详情索引。
...
2. GSC 已存在 `sitemap.xml`、`sitemap-crosshairs.xml` 与 `sitemap-images.xml` 提交记录；本轮发布后文件将自动更新为 1,248、1,024 与 1,124 个页面，下一步检查新增原创详情的抓取与 canonical。
