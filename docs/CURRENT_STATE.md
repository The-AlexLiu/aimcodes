...
- 5 种语言；
- 488 条准星源数据，去重后 480 种可见样式；职业选手同款允许共享外观但保留独立档案；
- 3 张地图预览；
- 3 轮反应测试；
- 9 个反应段位；
...
- 2,750 个生成 HTML 路由；
- 1,975 个 sitemap canonical URL；
- 336 个可索引准星详情，另有 152 个可访问但保持 `noindex,follow` 的目录详情；
- 336 张索引准星独立预览图、336 张对应 OG 图和 25 张集合页 OG 图；
- 独立准星 Sitemap 覆盖 1,680 个五语种准星详情 URL；
- 图片 Sitemap 覆盖 1,805 个五语种集合/详情页，共声明 3,485 个图片引用；
- 社媒视频、方形图和配套封面生成工具。
...
2. GSC 已存在 `sitemap.xml`、`sitemap-crosshairs.xml` 与 `sitemap-images.xml` 提交记录；本轮发布后文件将自动更新为 1,975、1,680 与 1,805 个页面，下一步检查新增原创详情的抓取与 canonical。
3. 观察 Funny 新增六类主题 14–28 天的查询、展示、排名、复制和分享，优先强化已经获得真实信号的主题，再决定下一批扩充方向。
