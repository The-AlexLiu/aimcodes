...
GA4 新增 `finder_nudge_view`、`finder_nudge_click`、`finder_nudge_dismiss`，并在进入测试的所有事件上下文中加入安全枚举 `finder_entry`（`nudge` 或 `direct`），用于计算浮窗曝光→点击→测试开始→完成→付费的漏斗；不发送搜索词、完整来源 URL 或任何个人数据。该功能尚未提交或发布，仍需桌面/手机真实浏览器验收与完整发布检查。

## 待验收：反应测试付费转化路径重排

反应测试结果页在当前功能分支中已将免费价值与付费交付重新分层：完成测试后先展示计分成绩、段位和通用瞄准风格，随后呈现五槽个人准星包与一次性 USD 1.99 行动入口。付款前使用遮罩代码和通用地图样例说明交付形式，不暴露实际交付的准星名称、逐款地图预览或完整代码；用户需选择模式、视觉偏好和主要武器，填写一次交付邮箱后再进入 Whop 结账。孤立的异常停顿会按 `browser-v4` 规则从三轮计分中排除，原始数据仍在折叠明细中显示并标记“未计入”。成绩明细、个人趋势、重新测试、挑战分享和免费准星库入口仍保持免费；低优先级内容默认折叠，手机标题与付费偏好步骤同步优化。该调整尚未发布，需完成本地桌面/手机验收及完整发布检查后再合并。
