...
- 1,364 个生成 HTML 路由；
- 744 个 sitemap canonical URL；
- 150 个可索引准星详情，另有 152 个可访问但保持 `noindex,follow` 的目录详情；
- 150 张索引准星独立预览图、150 张对应 OG 图和 15 张集合页 OG 图；
- 独立准星 Sitemap 覆盖 600 个四语种准星详情 URL；
- 图片 Sitemap 覆盖 660 个四语种集合/详情页，共声明 1,260 个图片引用；
- 社媒视频、方形图和配套封面生成工具。
...
- `pnpm validate:manifest` 统一检查完整目录、150 个索引记录和 15 个集合之间的关系；
...
- 索引详情由 100 个提升到 150 个，剩余 152 个详情继续保持 `noindex,follow`；
...
6. 职业选手候选库目前只有二手公开来源；在补足一手证据前不得创建可索引玩家页，也不得把候选日期描述为 AimCodes 已核验日期。

## 推荐后续顺序

1. 创建/进入 AdSense 账号，添加根域 `aimcodes.com`，再把 Google 提供的真实站点验证代码交给开发者安装并提交审核。
2. GSC 已存在 `sitemap.xml`、`sitemap-crosshairs.xml` 与 `sitemap-images.xml` 提交记录；本轮发布后文件将自动更新为 744、600 与 660 个页面，下一步检查四语种代表页面的抓取与 canonical。
