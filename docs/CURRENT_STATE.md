...
- 5 种语言；
- 408 条准星源数据，去重后 400 种可见样式；职业选手同款允许共享外观但保留独立档案；
- 3 张地图预览；
- 3 轮反应测试；
- 9 个反应段位；
...
- 2,340 个生成 HTML 路由；
- 1,565 个 sitemap canonical URL；
- 256 个可索引准星详情，另有 152 个可访问但保持 `noindex,follow` 的目录详情；
- 256 张索引准星独立预览图、256 张对应 OG 图和 25 张集合页 OG 图；
- 独立准星 Sitemap 覆盖 1,280 个五语种准星详情 URL；
- 图片 Sitemap 覆盖 1,405 个五语种集合/详情页，共声明 2,685 个图片引用；
- 社媒视频、方形图和配套封面生成工具。
...
- `pnpm validate:manifest` 统一检查完整目录、256 个索引记录和 25 个集合之间的关系；
...
- 本轮分享增长功能已通过桌面端、390px 手机端、四语种准星详情、地图与颜色还原、语言切换保参和反应挑战结果页的真实浏览器验收；自动检查覆盖 25 个 GA4 事件和五语种分享链接。
